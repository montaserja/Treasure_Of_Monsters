...
public class PlayerAttacks : NetworkBehaviour {
    ...
    void FireAttack()
    {
        GameObject bullet = (GameObject)Instantiate(effect3.gameObject, ShootingPoint.transform.position, cam.transform.rotation);
    ...
    void Update()
    {
        if (!isLocalPlayer)
            return;
        /*************************************************************/
        if (Input.GetKeyDown(KeyCode.Mouse0) && HasFirePower)
        {
            FireAttack();
        ...
        /*************************************************************/
        if (Input.GetKeyDown(KeyCode.Mouse0) && HasElectricPower)
        {
            ElectrecAttack();
            CmdDoElictric();
            if (isServer)
        ...
        /*************************************************************/
        if (Input.GetKeyDown(KeyCode.Mouse0) && HasWaterPower)
        {
            WaterAttack();
            CmdDoWater();
            if (isServer)
