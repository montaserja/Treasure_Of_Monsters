        ...
        public bool AssociateAssets()
        {
            ...
            for (int idx = 0; idx < m_instances.GetLength(0); idx++)
            {
                ...
                if (goInstance.m_desktopPrefab == null)
                {
                    if (!string.IsNullOrEmpty(goInstance.m_desktopPrefabFileName))
                    {
                        goInstance.m_desktopPrefab = Gaia.Utils.GetAsset(goInstance.m_desktopPrefabFileName, typeof(UnityEngine.GameObject)) as GameObject;
                        if (goInstance.m_desktopPrefab != null)
                        {
                            isModified = true;
                        }
                ...
                if (goInstance.m_mobilePrefab == null)
                {
                    if (!string.IsNullOrEmpty(goInstance.m_mobilePrefabFileName))
                    {
                        goInstance.m_mobilePrefab = Gaia.Utils.GetAsset(goInstance.m_mobilePrefabFileName, typeof(UnityEngine.GameObject)) as GameObject;
                        if (goInstance.m_mobilePrefab != null)
                        {
                            isModified = true;
                        }
