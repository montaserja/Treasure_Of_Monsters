...
namespace Gaia
{
    ...
    public class GaiaWorldManager
    {
        ...
        public void ExportWorldAsPng(string path)
        {
            ...
            {
                positionTU.x = srcX;
                for (int z = 0, srcZ = (int)m_worldBoundsTU.min.z; srcZ < (int)m_worldBoundsTU.max.z; z++, srcZ++)
                {
                    positionTU.z = srcZ;
            ...
            Gaia.Utils.CompressToSingleChannelFileImage(hm.Heights(), path, TextureFormat.RGBA32, true, false);
        }
        ...
        public void ExportSplatmapAsPng(string path, int textureIdx)
        {
            ...
            else
            {
                 ...
                 */
                Gaia.Utils.CompressToMultiChannelFileImage(splatMaps, path, TextureFormat.RGBA32, true, false);
            }
        }
        ...
        public void ExportGrassmapAsPng(string path)
        {
            ...
            Gaia.Utils.CompressToMultiChannelFileImage(detailMaps, path, TextureFormat.RGBA32, true, false);
        }
        ...
        public void ExportNormalmapAsPng(string path)
        {
            for (int tileX = 0; tileX < m_physicalTerrainArray.GetLength(0); tileX++)
            {
                for (int tileZ = 0; tileZ < m_physicalTerrainArray.GetLength(1); tileZ++)
                {
                    ...
                    Gaia.Utils.WriteAllBytes(spath, pngBytes);
                    MonoBehaviour.DestroyImmediate(nrmTexture);

                    AssetDatabase.Refresh();
                    Texture2D normalTex = Gaia.Utils.GetAsset(spath, typeof(Texture2D)) as Texture2D;
            ...
            for (int tileX = 0; tileX < m_physicalTerrainArray.GetLength(0); tileX++)
            {
                for (int tileZ = 0; tileZ < m_physicalTerrainArray.GetLength(1); tileZ++)
                {
                    ...
                    if (terrain != null)
                    {
                        ...
                        Gaia.Utils.CompressToMultiChannelFileImage(nrmMap, path + "_" + tileX + "_" + tileZ, TextureFormat.RGBA32, true, false);
#if UNITY_EDITOR
                        AssetDatabase.Refresh();
        ...
        public void ExportWaterflowMapAsPng(int iterations, string path)
        {
            for (int tileX = 0; tileX < m_physicalTerrainArray.GetLength(0); tileX++)
            {
                for (int tileZ = 0; tileZ < m_physicalTerrainArray.GetLength(1); tileZ++)
                {
                    ...
                    //Now write it to the path provided
                    Gaia.Utils.CompressToSingleChannelFileImage(hm.Heights(), path + "_" + tileX + "_" + tileZ + ".png", TextureFormat.RGBA32, true, false);
                }
            }
        }
        ...
        public void ExportShorelineMask(string path, float shoreHeightWU, float shoreWidthWU)
        {
            ...
            {
                positionTU.x = srcX;
                for (float z = 0, srcZ = m_worldBoundsTU.min.z; srcZ < m_worldBoundsTU.max.z; z += 1f, srcZ += 1f)
                {
                    positionTU.z = srcZ;
            ...
            //Now write it to the path provided
            Gaia.Utils.CompressToSingleChannelFileImage(shoreMask.Heights(), path, TextureFormat.RGBA32, true, false);
        }
            ...
            for (float x = minX; x < maxX; x += 1f)
            {
                ...
                for (float z = minZ; z < maxZ; z += 1f)
                {
                    ...
                    if (InBoundsTU(checkPos))
                    {
                        ...
                        {
                            strength = Gaia.Utils.Math_Distance(x, z, positionTU.x, positionTU.z) / maskSizeTU;
                            if (strength <= 1f)
                            {
        ...
        {
            if ( 
                (positionWU.x >= m_worldBoundsWU.min.x) &&
                (positionWU.z >= m_worldBoundsWU.min.z) &&
                (positionWU.x < m_worldBoundsWU.max.x) &&
                (positionWU.z < m_worldBoundsWU.max.z) )
            {
                return true;
            }
            else
        ...
        {
            if (
                (positionTU.x >= m_worldBoundsTU.min.x) &&
                (positionTU.z >= m_worldBoundsTU.min.z) &&
                (positionTU.x < m_worldBoundsTU.max.x) &&
                (positionTU.z < m_worldBoundsTU.max.z))
            {
                return true;
            }
            else
        ...
        {
            if (
                (positionNU.x >= m_worldBoundsNU.min.x) &&
                (positionNU.z >= m_worldBoundsNU.min.z) &&
                (positionNU.x < m_worldBoundsNU.max.x) &&
                (positionNU.z < m_worldBoundsNU.max.z))
            {
                return true;
            }
            else
        ...
        {
            return new Vector3(
                ((positionTU.x + m_TUZeroOffset.x) % m_worldBoundsTU.size.x),
                ((positionTU.y + m_TUZeroOffset.y) % m_worldBoundsTU.size.y),
                ((positionTU.z + m_TUZeroOffset.z) % m_worldBoundsTU.size.z)
            );
        }
        ...
        {
            return new Vector3(
                ((positionNU.x + m_NUZeroOffset.x) % 1f) * m_worldBoundsTU.size.x,
                ((positionNU.y + m_NUZeroOffset.y) % 1f) * m_worldBoundsTU.size.y,
                ((positionNU.z + m_NUZeroOffset.z) % 1f) * m_worldBoundsTU.size.z
                );
        }
