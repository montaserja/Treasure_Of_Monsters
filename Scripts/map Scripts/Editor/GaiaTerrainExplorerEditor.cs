...
namespace Gaia
{
    ...
    {
        private EditorUtils m_editorUtils = new EditorUtils();
        ...
        void OnEnable()
        {
            ...
            if (m_quantumStartHeights.Count == 0)
            {
                ...
                m_quantumStartHeights.Add(30f / m_terrainHeight);
                m_quantumCurves.Add(new AnimationCurve(new Keyframe(0f, 0.0f), new Keyframe(1.0f, 1.0f)));
                m_quantumStartHeights.Add(70f / m_terrainHeight);
                m_quantumCurves.Add(new AnimationCurve(new Keyframe(0f, 0.0f), new Keyframe(1.0f, 1.0f)));
                m_quantumStartHeights.Add(120f / m_terrainHeight);
                m_quantumCurves.Add(new AnimationCurve(new Keyframe(0f, 0.0f), new Keyframe(1.0f, 1.0f)));
                m_quantumStartHeights.Add(180f / m_terrainHeight);
                m_quantumCurves.Add(new AnimationCurve(new Keyframe(0f, 1.0f), new Keyframe(1.0f, 1.0f)));
            }
        }
        ...
        private void ExportMask()
        {
            ...
            if (Terrain.activeTerrain != null)
            {
                path = Path.Combine(path, Gaia.Utils.FixFileName(Terrain.activeTerrain.name + "_Mask"));
            }
            else
            {
                path = Path.Combine(path, Gaia.Utils.FixFileName("Mask"));
            }
            ...
            Gaia.Utils.CompressToMultiChannelFileImage(path, m_workingMask, m_workingMask, m_workingMask, m_workingMask, TextureFormat.RGBAFloat, GaiaConstants.ImageFileType.Exr);
        }
        ...
        private void SetupMask()
        {
            ...
            else
            {
                ...
                if (m_erosionMask1 == null || lastEditDT != m_erosionMaskDT1)
                {
                    ...
                    Gaia.Utils.MakeTextureReadable(m_erosionMaskTexture1);

                    //Make sure its not compressed
                    Gaia.Utils.MakeTextureUncompressed(m_erosionMaskTexture1);
                ...
                if (m_useErosionMask1)
                {
                    ...
                    {
                        if (!Gaia.Utils.Math_ApproximatelyEqual(m_erosionMaskStrength1, 1f))
                        {
                            workingMask1.MultiplyClamped(m_erosionMaskStrength1, 0f, 1f);
                        }
                        if (!Gaia.Utils.Math_ApproximatelyEqual(m_erosionMaskOffset1, 0f))
                        {
                            workingMask1.AddClamped(m_erosionMaskOffset1, 0f, 1f);
                        }
                        if (!Gaia.Utils.Math_ApproximatelyEqual(m_erosionMaskPowerOf1, 1f))
                        {
                            workingMask1.Power(m_erosionMaskPowerOf1);
                        }
                        if (!Gaia.Utils.Math_ApproximatelyEqual(m_erosionMaskContrast1, 1f))
                        {
                            workingMask1.Contrast(m_erosionMaskContrast1);
                        }
                        if (!Gaia.Utils.Math_ApproximatelyEqual(m_quantizeMask1, 0f))
                        {
            ...
            else
            {
                ...
                if (m_erosionMask2 == null || lastEditDT != m_erosionMaskDT2)
                {
                    ...
                    //Make sure its readable
                    Gaia.Utils.MakeTextureReadable(m_erosionMaskTexture2);

                    //Make sure its not compressed
                    Gaia.Utils.MakeTextureUncompressed(m_erosionMaskTexture2);
                ...
                if (m_useErosionMask2)
                {
                    ...
                    {
                        if (!Gaia.Utils.Math_ApproximatelyEqual(m_erosionMaskStrength2, 1f))
                        {
                    ...
                    for (int tileZ = 0; tileZ < tilesZ; tileZ++)
                    {
                        switch (m_operationType)
                        {
                            ...
                            case GaiaConstants.TerrainOperationType.HydraulicFilter:
                                ...
                                string fname = path;
                                fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_Sed"));
                                sedimentMap.Normalise();
                                Gaia.Utils.CompressToMultiChannelFileImage(fname, sedimentMap, sedimentMap, sedimentMap, sedimentMap, TextureFormat.RGBAFloat, GaiaConstants.ImageFileType.Exr);
...
//                                break;
        ...
        private void ExportHeightmap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                //path = Path.Combine(path, Gaia.Utils.FixFileName());
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    for (int tileZ = 0; tileZ < tilesZ; tileZ++)
                    {
                        ...
                        string fname = path;
                        fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_HeightMap"));
                        Gaia.Utils.CompressToMultiChannelFileImage(fname, exportHm, exportHm, exportHm, exportHm, TextureFormat.RGBAFloat, GaiaConstants.ImageFileType.Exr);
                    }
                }
            }
        }
        ...
        private void ExportFlowmap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_FlowMap"));
                        ...
                        Gaia.Utils.CompressToMultiChannelFileImage(fname, exportMap, exportMap, exportMap, exportMap, TextureFormat.RGBAFloat, GaiaConstants.ImageFileType.Exr);
                    }
                }
            }
        }
        ...
        private void ExportCuvaturemap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_CurvatureMap"));
        ...
        private void ExportSlopemap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_SlopeMap"));
        ...
        private void ExportAspectMap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_AspectMap"));
        ...
        private void ExportNormalmap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_NormalMap"));
                        ...
                        Gaia.Utils.WriteAllBytes(fname+".exr", exrBytes);
                        MonoBehaviour.DestroyImmediate(nrmTexture);

                        AssetDatabase.Refresh();
                        Texture2D normalTex = Gaia.Utils.GetAsset(fname, typeof(Texture2D)) as Texture2D;
        ...
        private void ExportNoisemap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_NoiseMap"));
        ...
        private void ExportBaseMap()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        string fname = path;
                        fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_BaseMap"));
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    if (maskValue > splatValue)
                    {
                        ...
                        float adjustment = 0f;
                        if (!Gaia.Utils.Math_ApproximatelyEqual(theRest, 0f))
                        {
        ...
        private void ZTest()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                for (int tileX = 0; tileX < tilesX; tileX++)
                {
                    ...
                    {
                        //string fname = path;
                        //fname = Path.Combine(path, Gaia.Utils.FixFileName(mgr.PhysicalTerrainArray[tileX, tileZ].name + "_Z"));
